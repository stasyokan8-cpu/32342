...
# ИСПРАВЛЕННАЯ ВЕРСИЯ: исправленные админ-функции, топ игроков, завершенные квесты
...
# -------------------------------------------------------------------
# ?? РАЗДЕЛ: АДМИН-ПАНЕЛЬ (РАСШИРЕННАЯ)
# -------------------------------------------------------------------
...
NEW_YEAR_QUIZ = [
    ...
    {"id": 23, "question": "??? В какой стране на Рождество зажигают свечи на венке из еловых веток?", "options": ["???? Германия", "???? Италия", "???? Греция", "???? Бразилия"], "correct": 0, "fact": "Рождественский венок со свечами — традиция, пришедшая из Германии."},
    ...
    {"id": 30, "question": "??? Что такое «Старый Новый год»?", "options": ["Новый год по лунному календарю", "Новый год по Юлианскому календарю", "Новый год для пессимистов", "Новый год в феврале"], "correct": 1, "fact": "Старый Новый год — это праздник, возникший из-за смены летоисчисления в 1918 году."},
    ...
    {"id": 33, "question": "????? Кто такие «Йоулупукки»?", "options": ["Финский Дед Мороз", "Рождественские гномы", "Пираты Северного полюса", "Олени Санта Клауса"], "correct": 0, "fact": "В Финляндии Деда Мороза зовут Йоулупукки, что дословно переводится как «Рождественский козел»."},
    ...
    {"id": 41, "question": "??? Какой праздник отмечают 7 января в России?", "options": ["Старый Новый год", "Рождество Христово", "Крещение", "День Святого Валентина"], "correct": 1, "fact": "7 января Русская Православная Церковь отмечает Рождество Христово по Юлианскому календарю."},
    ...
    {"id": 45, "question": "?? Как зовут снеговика, друга Эльзы из мультфильма «Холодное сердце»?", "options": ["Фрости", "Свен", "Олаф", "Кристофф"], "correct": 2, "fact": "Олаф — добрый и наивный снеговик, мечтающий о лете."},
...
# -------------------------------------------------------------------
# ?? РАЗДЕЛ: КВЕСТЫ (ПОЛНОСТЬЮ ИСПРАВЛЕННЫЕ)
# -------------------------------------------------------------------
async def show_quest_achievements(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    quest_achievements = [
        ("frozen_runes_completed", "?? Искатель замерзших рун", "Найти 3+ рун в квесте"),
    ...
    if not has_any:
        ...
        achievements_text += "Отправляйся в квесты через меню! ???"
...
"""
    ...
    keyboard = [
        [InlineKeyboardButton("?? Поиск рун", callback_data="quest_start_frozen_runes")],
...
# ?? Квест: Поиск замерзших рун (ИСПРАВЛЕННЫЙ)
async def quest_frozen_runes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    story = """?? <b>КВЕСТ: ПОИСК ЗАМЕРЗШИХ РУН</b>
...
<u>Цель:</u> Найти все 5 рун
<u>Награда:</u> 200 очков Санты + 100 опыта
<u>Время:</u> Неограничено
...
"""
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("?? Использовать магический компас (20 очков)", callback_data="quest_use_compass")],
        [InlineKeyboardButton("???>? Покинуть квест", callback_data="quest_leave")]
    ]
    
    await q.edit_message_text(story, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))

# ?? Квест: Спасение подарков (ИСПРАВЛЕННЫЙ)
...
<u>Цель:</u> Спасти 5 подарков
<u>Награда:</u> 250 очков Санты + 120 опыта
<u>Риск:</u> Потеря очков при провале
...
# ?? Квест: Поиск пропавших оленей (ИСПРАВЛЕННЫЙ)
async def quest_lost_reindeer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    story = """
?? <b>КВЕСТ: ПОИСК ПРОПАВШИХ ОЛЕНЕЙ</b>

Трое оленей Санты потерялись в снежной буре! 
Их имена: Искорка, Снежок и Комета.

<u>Цель:</u> Найти 3 оленей
<u>Награда:</u> 180 очков Санты + 80 опыта
<u>Особенность:</u> Каждый олень в своей локации
...
# ?? Квест: Штурм замка Гринча (ИСПРАВЛЕННЫЙ)
...
Внутри хранятся все украденные новогодние радости!

<u>Цель:</u> Пройти 3 уровня защиты
<u>Награда:</u> 300 очков Санты + 150 опыта + достижение
<u>Сложность:</b> Высокая
...
async def quest_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если нажата кнопка "Покинуть квест"
    if action == "leave":
        admin = is_admin(update)
        await q.edit_message_text(
            "???>? <b>Квест покинут</b>\n\n"
            "Ты решил вернуться в безопасное место. Может быть, в другой раз? ??",
            parse_mode='HTML',
            reply_markup=enhanced_menu_keyboard(admin)
        )
    ...
    if active_quest == "frozen_runes":
        ...
        if action == "frozen_path":
            ...
            if success:
                runes_found = random.randint(1, 2)
                quest_data["found_runes"] = min(quest_data["max_runes"], quest_data["found_runes"] + runes_found)
                points_earned = runes_found * 30
                exp_earned = runes_found * 15
                result = f"?? По заснеженной тропе ты нашёл {runes_found} рун! +{points_earned} очков, +{exp_earned} опыта"
            else:
            ...
            if success:
                runes_found = random.randint(2, 3)
                quest_data["found_runes"] = min(quest_data["max_runes"], quest_data["found_runes"] + runes_found)
                points_earned = runes_found * 40
                exp_earned = runes_found * 20
                result = f"?? В ледяной пещере ты нашёл {runes_found} рун! +{points_earned} очков, +{exp_earned} опыта"
            else:
                points_earned = -20
                result = "?? Ты попал в лавину! Потеряно 20 очков"
            
        elif action == "use_compass":
            if user_data[str(user.id)]["santa_points"] >= 20:
                add_santa_points(user.id, -20, context)
                quest_data["found_runes"] = min(quest_data["max_runes"], quest_data["found_runes"] + 1)
                result = f"?? Магический компас указал на руну! -20 очков за использование. Найдено: {quest_data['found_runes']}/{quest_data['max_runes']}"
            else:
                result = "?? Недостаточно очков Санты для использования компаса!"
        
        # Проверка завершения квеста
        if quest_data["found_runes"] >= quest_data["max_runes"] and not quest_data["completed"]:
            quest_data["completed"] = True
            points_bonus = 200
            exp_bonus = 100
            add_santa_points(user.id, points_bonus, context)
            add_reindeer_exp(user.id, exp_bonus)
            add_achievement(user.id, "frozen_runes_completed")
            user_data[str(user.id)]["quests_finished"] += 1
            
            result = f"""?? <b>КВЕСТ ЗАВЕРШЁН!</b>

?? Ты нашёл все {quest_data['max_runes']} замерзших рун!
? Бонусная награда: +{points_bonus} очков Санты
?? +{exp_bonus} опыта оленёнку
?? Получено достижение: "Искатель замерзших рун"

Новогодняя магия восстановлена! Волшебство вернулось в мир! ??"""
        ...
        if action == "disguise":
            success = random.random() > 0.4
            if success:
                gifts_found = random.randint(1, 2)
                quest_data["gifts_rescued"] = min(quest_data["max_gifts"], quest_data["gifts_rescued"] + gifts_found)
                quest_data["stealth"] = min(100, quest_data["stealth"] + 10)
                points_earned = gifts_found * 40
                exp_earned = gifts_found * 20
                result = f"?? Отличная маскировка! Ты прошёл незамеченным и спас {gifts_found} подарков. +{points_earned} очков, +{exp_earned} опыта"
            else:
                quest_data["stealth"] = max(0, quest_data["stealth"] - 15)
                result = "?? Стражи заметили тебя! Уровень скрытности понижен."
            
        elif action == "sneak":
            success = random.random() > 0.6
            if success:
                gifts_found = random.randint(2, 3)
                quest_data["gifts_rescued"] = min(quest_data["max_gifts"], quest_data["gifts_rescued"] + gifts_found)
                points_earned = gifts_found * 60
                exp_earned = gifts_found * 30
                result = f"? Молниеносный бросок! Ты спас {gifts_found} подарков. +{points_earned} очков, +{exp_earned} опыта"
            else:
                points_earned = -30
                quest_data["stealth"] = max(0, quest_data["stealth"] - 25)
                result = "?? Ты споткнулся и поднял тревогу! -30 очков"
            
        elif action == "attack":
            success = random.random() > 0.7
            if success:
                gifts_found = random.randint(3, 4)
                quest_data["gifts_rescued"] = min(quest_data["max_gifts"], quest_data["gifts_rescued"] + gifts_found)
                quest_data["stealth"] = 0
                points_earned = gifts_found * 80
                exp_earned = gifts_found * 40
                result = f"?? Мощная атака! Ты победил стражей и спас {gifts_found} подарков. +{points_earned} очков, +{exp_earned} опыта"
            else:
                points_earned = -50
                quest_data["stealth"] = 0
                result = "?? Ты был overpowered стражами! -50 очков"
        
        # Проверка завершения квеста
        if quest_data["gifts_rescued"] >= quest_data["max_gifts"] and not quest_data["completed"]:
            quest_data["completed"] = True
            points_bonus = 250
            exp_bonus = 120
            add_santa_points(user.id, points_bonus, context)
...
?? Ты спас все {quest_data['max_gifts']} подарков!
? Бонусная награда: +{points_bonus} очков Санты
?? +{exp_bonus} опыта оленёнку
?? Получено достижение: "Спасатель подарков"

Подарки возвращены детям! Радость Нового года спасена! ??"""
            quest_completed = True
    ...
    if quest_completed:
        # Если квест завершен, показываем кнопку возврата в меню
        keyboard.append([InlineKeyboardButton("?? Вернуться в меню", callback_data="back_menu")])
        # Удаляем данные квеста
...
async def enhanced_inline_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if q.data == "wish":
            ...
            await show_room_members_admin(update, context)
...
# -------------------------------------------------------------------
# ?? ИСПРАВЛЕННАЯ ФУНКЦИЯ ПОКАЗА УЧАСТНИКОВ КОМНАТЫ ДЛЯ АДМИНА
# -------------------------------------------------------------------
async def show_room_members_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    ...
    if is_admin(update):
        if not data["rooms"]:
            await q.answer("Нет созданных комнат!", show_alert=True)
            return
    ...
    if not user_room:
        await q.answer("Ты не в комнате!", show_alert=True)
        return
...
def main():
    ...
    # Добавляем обработчик для просмотра комнат админом
    app.add_handler(CallbackQueryHandler(enhanced_inline_handler, pattern="^admin_view_room_"))
    ...
    print("?? Бот v3.3 запускается на Replit...")
    print("? ВСЕ функции исправлены и улучшены!")
    print("?? Админ может просматривать участников любой комнаты - ? Исправлено")
    print("?? Топ игроков показывает имена - ? Исправлено")
    print("??? Квесты имеют начало и конец - ? Исправлено")
    print("?? Все мини-игры работают корректно")
