...
async def epic_grinch_battle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "grinch_fights" not in data["users"][user_id_str]:
        data["users"][user_id_str]["grinch_fights"] = 0
    if "grinch_wins" not in data["users"][user_id_str]:
        data["users"][user_id_str]["grinch_wins"] = 0
    
    # 🔥 Увеличиваем счетчик битв
    data["users"][user_id_str]["grinch_fights"] += 1
    
    # 🔥 Сохраняем данные
    save_all_data(data)
    
    print(f"✅ Статистика битвы обновлена: {user_id_str} - {data['users'][user_id_str]['grinch_fights']} битв")
...
async def battle_victory(update: Update, context: ContextTypes.DEFAULT_TYPE, battle_log):
    ...
    if "grinch_wins" not in data["users"][user_id_str]:
        data["users"][user_id_str]["grinch_wins"] = 0
    if "games_won" not in data["users"][user_id_str]:
        data["users"][user_id_str]["games_won"] = 0
    if "grinch_fights" not in data["users"][user_id_str]:
        data["users"][user_id_str]["grinch_fights"] = 0
    
    # 🔥 УВЕЛИЧИВАЕМ СЧЕТЧИКИ
    data["users"][user_id_str]["grinch_wins"] += 1
    data["users"][user_id_str]["games_won"] += 1
    
    # 🔥 СОХРАНЯЕМ ДАННЫЕ В ФАЙЛ
    save_all_data(data)
    
    print(f"✅ Статистика победы сохранена для пользователя {user_id_str}: {data['users'][user_id_str]['grinch_wins']} побед")
...
🏅 <b>Твоя статистика:</b>
• Всего битв с Гринчем: {data['users'][user_id_str]['grinch_fights']}
• Побед над Гринчем: {data['users'][user_id_str]['grinch_wins']}
• Общих побед в играх: {data['users'][user_id_str]['games_won']}
    ...
    try:
        ...
        if CONGRATS_AUDIO_URL.startswith("https"):
            try:
                ...
                print(f"✅ Музыкальная награда отправлена пользователю {user_id} по URL")
            except Exception as e:
                print(f"❌ Ошибка отправки аудио по URL пользователю {user_id}: {e}")
        
        # Способ 2: Отправка локального файла (для Replit)
        else:
            try:
                # Проверяем существование файла
                if os.path.exists(CONGRATS_AUDIO_URL):
                    ...
                    print(f"✅ Музыкальная награда отправлена пользователю {user_id} из файла")
                else:
                    print(f"❌ Файл {CONGRATS_AUDIO_URL} не найден")
                ...
                print(f"❌ Ошибка отправки аудио из файла пользователю {user_id}: {e}")
        ...
        print(f"❌ Критическая ошибка отправки музыкальной награды: {e}")
...
async def enhanced_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id_str not in data.get("users", {}):
        ...
        init_user_data(user.id)
        data = load_all_data()  # Перезагружаем данные
...
• Всего побед в играх: {user_info.get('games_won', 0)}
• Достижений: {len(user_info.get('achievements', []))}
"""
...
async def start_specific_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for giver, receiver in assigns.items():
        ...
        try:
            ...
            await context.bot.send_message(
                chat_id=int(giver),  # 🔥 Преобразуем в int
                text=f"🎁 <b>Тайный Санта запущен!</b> 🎄\n\n"
            ...
            failed_sends.append(room["members"][str(giver)]["name"])

    admin = is_admin(update)
    
    result_text = f"🎄 <b>Игра запущена в комнате {code}!</b> ✨\n\n"
    result_text += f"<b>Участников:</b> {len(members)}\n"
    result_text += f"<b>Сообщений отправлено:</b> {successful_sends}/{len(members)}\n"
    
    if failed_sends:
        result_text += f"<b>❌ Не отправлено:</b> {', '.join(failed_sends)}\n\n"
        result_text += f"<i>Эти пользователи не начали диалог с ботом или заблокировали его.</i>\n"
        result_text += f"<i>Попроси их написать /start боту.</i>\n\n"
    else:
        result_text += f"\nВсе участники получили своих получателей! 🎁\n\n"
    
    result_text += f"<b>💡 Информация для игроков:</b>\n"
    result_text += f"• Они видят только своего получателя\n"
    result_text += f"• Не видят, кто дарит им подарок\n"
    result_text += f"• Игра продолжается до вручения всех подарков"
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик ошибок"""
    print(f"⚠️ Ошибка в боте: {context.error}")
